...
**Status:** A proof of concept, an opening dialogue, and an invitation to collaborate. Intended to be hardened with a technical collaborator and legal counsel before first custody transfer.
...
Thus, a model preserved for reconstruction can be run, examined, situated in its lineage, and interpreted by future scholarship, even where regeneration is permanently foreclosed. The five records (§3) enclose exactly the material that makes reconstruction possible and that Pergamon can feasibly and lawfully hold; the heavier regeneration material (like training corpora, checkpoint series, optimizer states) is preserved *by reference* to separately-stewarded artifacts where it exists, marking the exact joint where institutional collabortion will fit.
...
Colon-separated `Key: Value`, UTF-8, one element per line (RFC 8493 §2.2.2). Standard reserved fields plus a `ARCHI-` namespace. Note: there is deliberately **no tape or count field here** (§1).
...
| `ARCHI-Process-Openness` | ✓ | Openness of the training process — roll-up of the ledger's training-material components (§7): `open-process`\|`weights-plus-partial`\|`weights-only`\|`undetermined` | `weights-only` |
| `ARCHI-Completeness` | ✓ | `complete`\|`partial`\|`weights-only` | `weights-only` |
...
`License-Class` and `Process-Openness` are **independent axes** — license openness and process openness are different questions, and the Profile records both.
...
The ARCHI format forbids bare `null`, because it conflates "the lab never released this" with "A Pergamon custodian failed to record it." To avoid this ambiguity, every component of an acquisition record carries an explicit **status** from a controlled vocabulary; any non-`present` status MUST carry a `reason` and SHOULD link to a `CUSTODIAN.txt` anchor.
...
- **`undetermined` has to show it looked.** Every `undetermined` carries a `review` reference — which review process ran, and when ("v0.1 review, 2026-06") — so the claim "we did our best" is checkable, and so a later determination reads as a dated improvement to the record rather than a contradiction of it. `undetermined` is process-relative and expected to change; the date is what marks *when we looked*.

**Ambiguity must be declared as an authored stance.** The bag-info fields most vulnerable to orphaned or provenance-broken models — `ARCHI-Origin-Jurisdiction`, `ARCHI-Release-Date`, `ARCHI-Parameter-Count`, `ARCHI-Weight-Precision` — MAY take the literal value `undetermined` rather than forcing a guess entered as fact. A confident fiction is worse than a marked gap.

**A reference is a preserved fossil, not a live link.** When a component is recorded as present-by-reference — most often the training corpus, which is typically far larger than the model and composed of third-party works held elsewhere — the reference must capture enough of the component to remain meaningful *after its target is gone*: the component's identity, composition, scale, source description, and its status at the time of capture. A reference that is merely a URL rots exactly like the material it points to; a preserved *description* of a corpus (what it contained, how it was assembled, where it lived, under what terms) remains a genuine historical record even once the corpus itself becomes unrecoverable. The pointer is a fossil of the thing, not a hyperlink to it.
...
```json
{
  ...
  "components": {
    ...
    "evaluation_results":      { "status": "unrecoverable",
                                 ...
                                 "custodian_note": "ARCHI/CUSTODIAN.txt#eval" }
  },
  "rollup": { "completeness": "weights-only", "process_openness": "weights-only" }
}
```

`ARCHI-Completeness` and `ARCHI-Process-Openness` in `bag-info.txt` are the human-facing roll-ups of this ledger; the ledger is the auditable detail behind them. `Process-Openness` in particular summarizes the ledger's training-material components (`training_data`, `training_code`, checkpoints): if those are present or preserved-by-reference, the release is `open-process`; if withheld, `weights-only`. The full references themselves live in the ledger, following the preserved-fossil discipline above — the roll-up points at the pointers; it never replaces them.

## 8. The Interpretive Layer

The `ARCHI/` files that describe and locate each model (besides the completeness ledger in §7). The field sets below are v0.1 and expected to firm up with a technical collaborator; the structure is stable.
...
```json
{
  ...
  "selection_rationale": "First proof: permissive, open-process Western models with low custody risk.",
  "bags": [
    { "path": "archimedes-olmo-7b",
      "model_id": "allenai/OLMo-7B",
      "payload_oxum": "13958643712.4",
      "license_class": "permissive",
      "process_openness": "open-process",
...
- **Multi-tape models — mostly a referencing question, not a spanning one.** Analysis of the fully-open cases (OLMo-class) shows the bulk lives in training corpora and checkpoint series, not final weights — so most "too big for one tape" pressure dissolves into compact bags *referencing* separately-preserved heavy artifacts under the preserved-fossil discipline (§7), with `Process-Openness` as the roll-up. A `Bag-Group-Identifier` / spanned-bag rule remains the fallback for the rare bag that genuinely exceeds a cartridge; design both in v0.2, referencing-first.
