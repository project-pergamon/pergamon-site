...
baginfo = [
 ...
 ("ARCHI-Process-Openness", "open-process"),
 ("ARCHI-Completeness", "complete"),
...
w(os.path.join(PERG, "PROVENANCE.json"), json.dumps({
  ...
  "process_openness_basis": {
    "status": "yes",
    "basis": "Ai2 released training code, data (Dolma), and checkpoints/logs, so the model can in principle be retrained from recipe rather than only copied."
  }
}, indent=2))
...
w(os.path.join(PERG, "COMPLETENESS.json"), json.dumps({
  ...
  "rollup": {"completeness": "complete", "process_openness": "open-process"},
...
w(os.path.join(TAPE, "ARCHIMEDES-TAPE.json"), json.dumps({
  ...
  "bags": [
    {"path": "archimedes-olmo-7b", "model_id": "allenai/OLMo-7B",
     "payload_oxum": oxum, "license_class": "permissive", "process_openness": "open-process",
